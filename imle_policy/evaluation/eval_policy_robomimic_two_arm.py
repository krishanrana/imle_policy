...
import cv2
from imle_policy.dataloaders.dataset_robomimic_hdf5 import normalize_data, unnormalize_data
...
def process_obs(obs):
    ...
    pos_array_0 = obs['robot0_eef_pos']
    quat_array_0 = obs['robot0_eef_quat']
    quat_array_0 = quaternion_to_rotation_6D(quat_array_0)
    gripper_array_0 = obs['robot0_gripper_qpos']

    pos_array_1 = obs['robot1_eef_pos']
    quat_array_1 = obs['robot1_eef_quat']
    quat_array_1 = quaternion_to_rotation_6D(quat_array_1)
    gripper_array_1 = obs['robot1_gripper_qpos']

    p_obs = np.concatenate([pos_array_0, quat_array_0, gripper_array_0, pos_array_1, quat_array_1, gripper_array_1])
    
    front_image_0 = np.moveaxis(obs['shouldercamera0_image'][::-1], -1, 0)
    hand_image_0 = np.moveaxis(obs['robot0_eye_in_hand_image'][::-1], -1, 0)
    front_image_1 = np.moveaxis(obs['shouldercamera1_image'][::-1], -1, 0)
    hand_image_1 = np.moveaxis(obs['robot1_eye_in_hand_image'][::-1], -1, 0)

    obs = {
        'obs': p_obs,
        'front_image_0': front_image_0,
        'hand_image_0': hand_image_0,
        'front_image_1': front_image_1,
        'hand_image_1': hand_image_1}
...
def evaluate(args, nets, stats, method='rs_imle'):
    ...
    env = suite.make(
        ...
        use_camera_obs=True,
        camera_names = ['robot0_eye_in_hand', 'robot1_eye_in_hand', 'shouldercamera0', 'shouldercamera1'], 
        camera_heights = 84,
        camera_widths = 84,
    ...
    for i in tqdm(range(args['num_trails'])): 
        ...
        while not done:
            ...
            front_image_0 = np.stack([x['front_image_0'] for x in obs_deque])
            hand_image_0 = np.stack([x['hand_image_0'] for x in obs_deque])
            front_image_1 = np.stack([x['front_image_1'] for x in obs_deque])
            hand_image_1 = np.stack([x['hand_image_1'] for x in obs_deque])

            obs = np.stack([x['obs'] for x in obs_deque])

            # normalize observation
            nobs = normalize_data(obs, stats=stats['obs'])
            nimage_front_0 = front_image_0/255.0
            nimage_hand_0 = hand_image_0/255.0
            nimage_front_1 = front_image_1/255.0
            nimage_hand_1 = hand_image_1/255.0

            # device transfer
            nimage_front_0 = torch.from_numpy(nimage_front_0).to(device, dtype=torch.float32)
            nimage_hand_0 = torch.from_numpy(nimage_hand_0).to(device, dtype=torch.float32)
            nimage_front_1 = torch.from_numpy(nimage_front_1).to(device, dtype=torch.float32)
            nimage_hand_1 = torch.from_numpy(nimage_hand_1).to(device, dtype=torch.float32)
            ...
            with torch.no_grad():
                # get image features
                image_features_front_0 = nets['vision_encoder_0'](nimage_front_0)
                image_features_hand_0 = nets['vision_encoder_1'](nimage_hand_0)
                image_features_front_1 = nets['vision_encoder_2'](nimage_front_1)
                image_features_hand_1 = nets['vision_encoder_3'](nimage_hand_1)
