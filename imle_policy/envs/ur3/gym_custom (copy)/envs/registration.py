...
from imle_policy.envs.ur3.gym_custom import error, logger
    ...
    def spec(self, path):
        if ':' in path:
            ...
            # catch ImportError for python2.7 compatibility
                ...
                raise error.Error('A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `gym_custom.make()`'.format(mod_name))
        else:
            id = path
